package com.cloudia.backend.CM_03_1003.service.impl;
...
import com.cloudia.backend.CM_03_1003.mapper.CM031003Mapper;
...
public class CM031003ServiceImpl implements CM031003Service {
    ...
    public ResponseEntity<ResponseModel<List<ProductInfo>>> getProductDetail(Long productId) {
        try {
            List<ProductInfo> list = Optional.ofNullable(cm031003Mapper.selectProductDetail(productId))
                .map(Collections::singletonList)
                .orElse(Collections.emptyList());
            log.info(CM031003MessageConstant.PRODUCT_FIND_BY_ID_COMPLETE, productId, list.size());
            return ResponseEntity.ok(setResponseDto(list, true, CM031003MessageConstant.SUCCESS_PRODUCT_FIND));
        } catch (Exception e) {
    ...
    public ResponseEntity<ResponseModel<Void>> addToCart(CartRequest cartRequest) {
        try {
            if (cm031003Mapper.findCartItem(cartRequest.getUserId(), cartRequest.getProductId()) != null) {
                cm031003Mapper.updateQuantity(
                    cartRequest.getUserId(),
                    cartRequest.getProductId(),
                    cartRequest.getQuantity()
                );
            } else {
                cm031003Mapper.insertCartItem(
                    cartRequest.getUserId(),
                    cartRequest.getProductId(),
                    cartRequest.getQuantity()
                );
            ...
            log.error(CMMessageConstant.ERROR_DATABASE + " - {}", e.getMessage(), e);
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                .body(setResponseDto(null, false, CM031003MessageConstant.FAIL_CART_ADD));
        }
    }
    ...
    public ResponseEntity<ResponseModel<List<Categories>>> findAllCategoryGroupCode() {
        try {
            List<Categories> categoryGroupCodeList = cm031003Mapper.findAllCategoryGroupCode();
            log.info(CM031003MessageConstant.CATEGORY_GROUP_FETCH_SUCCESS, categoryGroupCodeList == null ? 0 : categoryGroupCodeList.size());

            return ResponseEntity.ok(createResponseModel(categoryGroupCodeList, true, CM031003MessageConstant.CATEGORY_GROUP_FETCH_SUCCESS));
        } catch (DataAccessException dae) {
            log.error(CM031003MessageConstant.CATEGORY_GROUP_FETCH_DB_ERROR, dae.getMessage(), dae);
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                    .body(createResponseModel(Collections.emptyList(), false, CMMessageConstant.ERROR_DATABASE));
        } catch (NullPointerException npe) {
            log.error(CM031003MessageConstant.CATEGORY_GROUP_FETCH_NULL, npe.getMessage(), npe);
            return ResponseEntity.status(HttpStatus.BAD_REQUEST)
                    .body(createResponseModel(Collections.emptyList(), false, CMMessageConstant.MESSAGE_NULL_NOT_ALLOWED));
        } catch (Exception e) {
            log.error(CM031003MessageConstant.CATEGORY_GROUP_FETCH_ERROR, e.getMessage(), e);
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
    ...
    public ResponseEntity<ResponseModel<List<CategoryDetails>>> findCategory(List<String> categoryGroupCode) {
        try {
            List<CategoryDetails> CategoryDetailList = cm031003Mapper.findCategory(categoryGroupCode);
            log.info(CM031003MessageConstant.CATEGORY_DETAIL_FETCH_SUCCESS, CategoryDetailList == null ? 0 : CategoryDetailList.size());

            return ResponseEntity.ok(createResponseModel(CategoryDetailList, true, CM031003MessageConstant.CATEGORY_DETAIL_FETCH_SUCCESS));
        } catch (DataAccessException dae) {
            log.error(CM031003MessageConstant.CATEGORY_DETAIL_FETCH_DB_ERROR, dae.getMessage(), dae);
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                    .body(createResponseModel(Collections.emptyList(), false, CMMessageConstant.ERROR_DATABASE));
        } catch (NullPointerException npe) {
            log.error(CM031003MessageConstant.CATEGORY_DETAIL_FETCH_NULL, npe.getMessage(), npe);
            return ResponseEntity.status(HttpStatus.BAD_REQUEST)
                    .body(createResponseModel(Collections.emptyList(), false, CMMessageConstant.MESSAGE_NULL_NOT_ALLOWED));
        } catch (Exception e) {
            log.error(CM031003MessageConstant.CATEGORY_DETAIL_FETCH_ERROR, e.getMessage(), e);
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                    .body(createResponseModel(Collections.emptyList(), false, CMMessageConstant.ERROR_INTERNAL_SERVER));
        }
    }
    ...
    public ResponseEntity<ResponseModel<List<CategoryGroupForCheckbox>>> findAllCategoryGroupForCheckbox() {
        try {
            ...
            return ResponseEntity.ok(createResponseModel(result, true, CM031003MessageConstant.CATEGORY_GROUP_FETCH_SUCCESS));
        } catch (Exception e) {
            log.error(CM031003MessageConstant.CATEGORY_GROUP_FETCH_ERROR, e.getMessage(), e);
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                    .body(createResponseModel(Collections.emptyList(), false, CMMessageConstant.ERROR_INTERNAL_SERVER));
    ...
    public ResponseEntity<ResponseModel<List<ProductInfo>>> getProductList(List<String> categories) {
        try {
            List<ProductInfo> list = Optional.ofNullable(cm031003Mapper.selectNewProductList(categories))
                .orElse(Collections.emptyList());
            return ResponseEntity.ok(setResponseDto(list, true, CM031003MessageConstant.SUCCESS_PRODUCT_FIND));
        } catch (Exception e) {
            log.error(CMMessageConstant.ERROR_DATABASE + " - {}", e.getMessage(), e);
