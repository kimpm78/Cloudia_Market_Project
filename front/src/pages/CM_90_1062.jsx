...
const ImageUploadField = ({ id, label, file, preview, onChange, error, currentImageUrl }) => (
  <div className="mb-3">
    ...
    <div className="text-muted small mt-1">
      ※最小サイズは<b>200 X 200 ピクセル</b>以上を推奨します。
    </div>
    {error && <div className="text-danger">{error}</div>}
  </div>
);
...
const DetailImageUpload = ({ images, onChange, error, onError, isEdit = false }) => {
  ...
  return (
    <div className="mb-3">
      ...
      >
        ...
        {isDragging && (
          ...
          >
            <div className="text-center">
              ...
              <p className="mt-2 mb-0 text-primary fw-bold">画像をここにドロップしてください</p>
            </div>
          </div>
        )}
        ...
        {images.length === 0 ? (
          <div className="text-center py-4 text-muted">
            ...
            <p className="mt-2 mb-0">アップロードされた画像がありません。</p>
            <p className="mb-0">
              <small>ファイルをドラッグするか、「アップロード」ボタンをクリックしてください</small>
            </p>
          </div>
        ) : (
          images.map((image) => (
            <div key={image.id} className="d-flex align-items-center p-2 border-bottom">
              ...
              <div className="flex-grow-1">
                ...
                <small className="text-muted">
                  {image.file ? `${(image.file.size / 1024).toFixed(1)} KB` : '既存画像'}
                  {image.isNew && <span className="badge bg-success ms-2">新規画像</span>}
                </small>
              </div>
      ...
      {/* 画像一覧 */}
      <div>
        ※最小サイズは<b>150 X 150 ピクセル</b>以上を推奨します。
      </div>
...
export default function CM_90_1062() {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        if (isMounted) {
          open('error', 'データの読み込み中にエラーが発生しました。');
        }
      } finally {
        if (isMounted) {
  ...
  const validateForm = useCallback(() => {
    ...
    if (!form.productName) {
      newErrors.productName = '商品名を入力してください。';
    }

    if (PRODUCT_CATEGORY_LABEL[1] === form.productCategory) {
      if (form.expectedDeliveryDate) {
        newErrors.expectedDeliveryDate = '出荷予定日を空にしてください。';
      }
      if (form.reservationDeadline) {
        newErrors.reservationDeadline = '予約締切日を空にしてください。';
      }
    } else if (PRODUCT_CATEGORY_LABEL[2] === form.productCategory) {
      if (!form.expectedDeliveryDate) {
        newErrors.expectedDeliveryDate = '出荷予定日を入力してください。';
      }
      if (!form.reservationDeadline) {
    ...
    if (!form.purchaseLimit) {
      newErrors.purchaseLimit = '購入上限数を入力してください。';
    } else if (form.purchaseLimit > MAX_INT) {
      newErrors.purchaseLimit = '購入上限数が上限を超えています。';
    }
    if (!form.weight) {
  ...
  return (
    <div className="d-flex flex-grow-1">
      <div className="content-wrapper p-3">
        <h5 className="border-bottom pb-2 mb-3">商品修正</h5>
