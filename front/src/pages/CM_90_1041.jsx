...
export default function CM_90_1041() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      var result = await axiosInstance.get('/admin/menu/banner/availableDisplayOrders');

      if (result.data.result) {
        setAvailableOrders(result.data.resultList);
        if (result.data.resultList.length > 0) {
          setForm((prevForm) => ({
            ...prevForm,
            displayOrder: result.data.resultList[0],
          }));
        }
      } else {
  ...
  const handleRegisterClick = async (e) => {
    if (validateForm()) {
      ...
      const result = await axiosInstance.post('/admin/menu/banner/upload', formData);
  ...
  return (
    <div className="d-flex">
      <div className="content-wrapper p-3">
        <h2 className="border-bottom pb-2 mb-3">バナー登録</h2>
        <form>
          {/* バナー名 */}
          <div className="mb-3">
